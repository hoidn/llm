...
def mock_app():
    ...
    app.handle_task_command = AsyncMock()
...
@pytest.mark.asyncio
async def test_orchestrator_run_completes_with_stub_logic(mock_app): # Renamed for clarity
    """
...
@pytest.mark.asyncio
async def test_generate_plan_success(mock_app):
    orchestrator = CodingWorkflowOrchestrator(
        app=mock_app,
    ...
    success = await orchestrator._generate_plan()
    ...
    mock_app.handle_task_command.assert_awaited_once_with(
        "user:generate-plan-from-goal",
        params={"goal": "Test Goal Plan", "context_string": "Test Context Plan"}
    )

@pytest.mark.asyncio
async def test_generate_plan_task_fails(mock_app):
...
@pytest.mark.asyncio
async def test_generate_plan_parsing_fails(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
    mock_app.handle_task_command.return_value = {
        "status": "COMPLETE",
...
@pytest.mark.asyncio
async def test_generate_plan_app_call_exception(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
...
@pytest.mark.asyncio
async def test_execute_code_success(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
    ...
    task_result_obj = await orchestrator._execute_code()
...
@pytest.mark.asyncio
async def test_execute_code_aider_fails(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
    ...
    mock_app.handle_task_command.assert_awaited_once()

@pytest.mark.asyncio
async def test_execute_code_no_plan(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
    ...
    mock_app.handle_task_command.assert_not_awaited()

@pytest.mark.asyncio
async def test_execute_code_plan_missing_instructions(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
    ...
    mock_app.handle_task_command.assert_not_awaited() # Should not call if plan is invalid

@pytest.mark.asyncio
async def test_execute_code_app_call_exception(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "t")
...
@pytest.mark.asyncio
async def test_validate_code_tests_pass(mock_app):
    ...
    task_result_obj = await orchestrator._validate_code()
...
@pytest.mark.asyncio
async def test_validate_code_tests_fail(mock_app):
...
@pytest.mark.asyncio
async def test_validate_code_shell_command_execution_fails(mock_app):
...
@pytest.mark.asyncio
async def test_validate_code_no_test_command(mock_app):
...
@pytest.mark.asyncio
async def test_validate_code_app_call_exception(mock_app):
...
@pytest.mark.asyncio
async def test_analyze_iteration_suggests_success(mock_app, mock_aider_result_success, mock_test_result_pass):
    ...
    analysis_decision = await orchestrator._analyze_iteration(mock_aider_result_success, mock_test_result_pass)
...
@pytest.mark.asyncio
async def test_analyze_iteration_suggests_retry(mock_app, mock_aider_result_success, mock_test_result_fail):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    orchestrator.iteration = 1
    orchestrator.current_plan = DevelopmentPlan(instructions="Plan A", files=["a.py"], test_command="test_cmd")
    ...
    analysis_decision = await orchestrator._analyze_iteration(mock_aider_result_success, mock_test_result_fail)
...
@pytest.mark.asyncio
async def test_analyze_iteration_suggests_failure(mock_app, mock_aider_result_failure, mock_test_result_fail):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    ...
    analysis_decision = await orchestrator._analyze_iteration(mock_aider_result_failure, mock_test_result_fail)
...
@pytest.mark.asyncio
async def test_analyze_iteration_llm_task_fails(mock_app, mock_aider_result_success, mock_test_result_pass):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    orchestrator.iteration = 1
    orchestrator.current_plan = DevelopmentPlan(instructions="Plan A", files=["a.py"], test_command="test_cmd")
...
@pytest.mark.asyncio
async def test_analyze_iteration_llm_returns_invalid_verdict_structure(mock_app, mock_aider_result_success, mock_test_result_pass):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    orchestrator.iteration = 1
    orchestrator.current_plan = DevelopmentPlan(instructions="Plan A", files=["a.py"], test_command="test_cmd")
...
@pytest.mark.asyncio
async def test_run_success_first_iteration(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)

    with patch.object(orchestrator, '_generate_plan', return_value=True) as mock_gen_plan, \
         patch.object(orchestrator, '_execute_code', new_callable=AsyncMock) as mock_exec_code, \
         patch.object(orchestrator, '_validate_code', new_callable=AsyncMock) as mock_validate_code, \
         patch.object(orchestrator, '_analyze_iteration', new_callable=AsyncMock) as mock_analyze:
        ...
        mock_gen_plan.assert_called_once()
        mock_exec_code.assert_awaited_once()
        mock_validate_code.assert_awaited_once()
        mock_analyze.assert_awaited_once()

@pytest.mark.asyncio
async def test_run_one_retry_then_success(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
...
@pytest.mark.asyncio
async def test_run_max_retries_then_fail(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", max_retries=2)

    with patch.object(orchestrator, '_generate_plan', return_value=True) as mock_gen_plan, \
         patch.object(orchestrator, '_execute_code', new_callable=AsyncMock, return_value=TaskResult(status="COMPLETE", content="Aider Output", notes={})) as mock_exec_code, \
         patch.object(orchestrator, '_validate_code', new_callable=AsyncMock, return_value=TaskResult(status="COMPLETE", content="Tests Fail", notes={"exit_code": 1})) as mock_validate_code, \
         patch.object(orchestrator, '_analyze_iteration', new_callable=AsyncMock) as mock_analyze:
...
@pytest.mark.asyncio
async def test_run_initial_plan_generation_fails(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    with patch.object(orchestrator, '_generate_plan', new_callable=AsyncMock, return_value=False) as mock_gen_plan, \
         patch.object(orchestrator, '_execute_code', new_callable=AsyncMock) as mock_exec_code: # Also mock other phases to check not called
        ...
        mock_gen_plan.assert_awaited_once()
        mock_exec_code.assert_not_awaited()

@pytest.mark.asyncio
async def test_run_analysis_suggests_direct_failure(mock_app):
    orchestrator = CodingWorkflowOrchestrator(mock_app, "g", "c", "test_cmd", 3)
    with patch.object(orchestrator, '_generate_plan', return_value=True) as mock_gen_plan, \
         patch.object(orchestrator, '_execute_code', new_callable=AsyncMock, return_value=TaskResult(status="COMPLETE", content="Aider V1", notes={})) as mock_exec_code, \
         patch.object(orchestrator, '_validate_code', new_callable=AsyncMock, return_value=TaskResult(status="COMPLETE", content="Tests Fail V1", notes={"exit_code": 1})) as mock_validate_code, \
         patch.object(orchestrator, '_analyze_iteration', new_callable=AsyncMock) as mock_analyze:
