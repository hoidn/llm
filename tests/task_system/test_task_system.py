...
def test_register_template_missing_required_fields(task_system, caplog):
    ...
    template_no_name = VALID_ATOMIC_TEMPLATE.copy(); del template_no_name["name"]
    template_no_type = VALID_ATOMIC_TEMPLATE.copy(); del template_no_type["type"]
    template_no_subtype = VALID_ATOMIC_TEMPLATE.copy(); del template_no_subtype["subtype"]

    with caplog.at_level(logging.ERROR):
        task_system.register_template(template_no_name)
        assert "Missing 'name', 'type', or 'subtype'" in caplog.text
        assert "test_atomic_task" not in task_system.templates # Check name wasn't added
...
def test_register_template_non_atomic_warns(task_system, caplog):
    ...
    assert task_system.template_index[f"{template['type']}:{template['subtype']}"] == template["name"]
...
def test_register_template_missing_params_warns(task_system, caplog):
    ...
    with caplog.at_level(logging.WARNING):
        task_system.register_template(template)
        assert f"Template '{template['name']}' registered without a 'params' attribute" in caplog.text
...
def test_register_template_overwrites(task_system):
    ...
    assert task_system.templates["test_atomic_task"]["description"] == "Updated description"
...
def test_find_template_ignores_non_atomic_by_type_subtype(task_system):
    ...
    found = task_system.find_template(f"{composite_template['type']}:{composite_template['subtype']}")
    assert found is None

    # Search for the atomic type:subtype - should be found
    found = task_system.find_template(f"{atomic_template['type']}:{atomic_template['subtype']}")
    assert found is not None
    assert found == atomic_template
...
def test_execute_atomic_template_deferred(task_system):
    ...
    with pytest.raises(NotImplementedError, match="execute_atomic_template implementation deferred"):
...
def test_generate_context_for_memory_system_deferred(task_system):
    """Verify deferred method raises NotImplementedError."""
    with pytest.raises(NotImplementedError, match="generate_context_for_memory_system implementation deferred"):
        task_system.generate_context_for_memory_system(MagicMock(), {}) # Mock input, empty index
...
def test_resolve_file_paths_deferred(task_system, mock_memory_system):
    ...
    result = task_system.resolve_file_paths({}, mock_memory_system, MagicMock()) # Empty template, mock deps
