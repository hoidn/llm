...
def llm_manager_instance(mock_agent_instance):
    ...
    with patch('src.handler.llm_interaction_manager.Agent', new_callable=MagicMock) as MockAgentClassForFixture:
        MockAgentClassForFixture.return_value = mock_agent_instance # Configure the mock instance returned
...
def test_llm_manager_init_agent_exception(mock_log_error):
    ...
    # Use new_callable=MagicMock to ensure the mock class itself is truthy
    with patch('src.handler.llm_interaction_manager.Agent', new_callable=MagicMock) as MockAgentClassForTest:
        MockAgentClassForTest.side_effect = test_exception # Make Agent() raise an error
...
def test_manager_execute_call_success(llm_manager_instance, mock_agent_instance):
    ...
    mock_agent_instance.run_sync.return_value = mock_response # Ensure mock agent returns this
