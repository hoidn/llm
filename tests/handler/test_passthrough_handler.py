...
def passthrough_handler(mock_task_system, mock_memory_system, mocker):
    ...
    # Use AsyncMock for the now-async get_relevant_files method
    mock_file_context_manager.get_relevant_files = AsyncMock(return_value=['/mock/file.py'])
    ...
    # Mock execute_call as AsyncMock
    mock_llm_manager.execute_call = AsyncMock(return_value=TaskResult(
        status="COMPLETE",
        content="Passthrough Response", # Keep content consistent with test assertion
        notes={"usage": {"prompt_tokens": 10, "completion_tokens": 20}, "tool_calls": None}
    ))
    ...
    with patch('src.handler.base_handler.FileContextManager', return_value=mock_file_context_manager), \
        ...
        mocker.patch('src.handler.command_executor.execute_command_safely', return_value={'success': True, 'stdout': '/path/from/cmd', 'stderr': '', 'exit_code': 0})
        ...
        # --- Mock async methods on BaseHandler ---
        handler._execute_llm_call = AsyncMock(return_value=TaskResult(
            status="COMPLETE",
            content="LLM call response",
            notes={}
        ))
        handler._execute_tool = AsyncMock(return_value=TaskResult(
            status="COMPLETE",
...
@pytest.mark.asyncio
async def test_init_registers_command_tool(passthrough_handler):
...
@pytest.mark.asyncio
async def test_handle_query_success(passthrough_handler, mocker):
    ...
    passthrough_handler.prime_data_context = AsyncMock(return_value=True)
    ...
    passthrough_handler._create_new_subtask = AsyncMock(return_value=mock_subtask_result)
    ...
    # Act
    result = await passthrough_handler.handle_query(query)
    ...
    # Assert delegation
    passthrough_handler.prime_data_context.assert_awaited_once_with(query=query)
    passthrough_handler._create_new_subtask.assert_awaited_once_with(query=query)
...
@pytest.mark.asyncio
async def test_handle_query_prime_data_context_failure(passthrough_handler, mocker):
    """Test query handling when prime_data_context fails."""
    query = "Query causing priming failure"
    passthrough_handler.prime_data_context = AsyncMock(return_value=False)
    passthrough_handler.conversation_history = []
...
@pytest.mark.asyncio
async def test_handle_query_no_active_subtask_creates_new(passthrough_handler, mocker):
    """Test that handle_query calls _create_new_subtask when no active subtask."""
    query = "New query"
    passthrough_handler.prime_data_context = AsyncMock(return_value=True)
    passthrough_handler._create_new_subtask = AsyncMock(return_value=TaskResult(status="COMPLETE", content="created"))
    passthrough_handler._continue_subtask = AsyncMock() # Should not be called
    passthrough_handler.active_subtask_id = None

    await passthrough_handler.handle_query(query)

    passthrough_handler._create_new_subtask.assert_awaited_once_with(query=query)
    passthrough_handler._continue_subtask.assert_not_awaited()


@pytest.mark.asyncio
async def test_handle_query_active_subtask_continues(passthrough_handler, mocker):
    """Test that handle_query calls _continue_subtask when an active subtask exists."""
    query = "Continuation query"
    passthrough_handler.prime_data_context = AsyncMock(return_value=True)
    passthrough_handler._create_new_subtask = AsyncMock() # Should not be called
    passthrough_handler._continue_subtask = AsyncMock(return_value=TaskResult(status="COMPLETE", content="continued"))
    passthrough_handler.active_subtask_id = "test_subtask_123"

    await passthrough_handler.handle_query(query)

    passthrough_handler._continue_subtask.assert_awaited_once_with(query=query)
    passthrough_handler._create_new_subtask.assert_not_awaited()


@pytest.mark.asyncio
async def test_handle_query_notes_include_relevant_files_from_context(passthrough_handler, mocker): # mocker might not be needed here
    ...
    async def prime_data_context_side_effect(query: Optional[str] = None, initial_files: Optional[List[str]] = None): # Match signature
    ...
    with patch.object(passthrough_handler, 'prime_data_context', side_effect=prime_data_context_side_effect) as mock_prime_dc, \
        ...
        mock_prime_dc.assert_awaited_once_with(query=query)
        mock_create_subtask.assert_awaited_once_with(query=query) # Assuming active_subtask_id is None
...
@pytest.mark.asyncio
async def test_handle_query_unexpected_exception(passthrough_handler, mocker):
    """Test handle_query when an unexpected exception occurs during priming."""
    query = "Query causing unexpected error"
    passthrough_handler.prime_data_context = AsyncMock(side_effect=RuntimeError("Unexpected priming error"))
...
@pytest.mark.asyncio
async def test_create_new_subtask_success(passthrough_handler, mocker):
    ...
    passthrough_handler._execute_llm_call = AsyncMock(return_value=mock_llm_response)

    # Pre-set data_context as prime_data_context is called in handle_query
    passthrough_handler.data_context = MagicMock() 
    passthrough_handler.data_context.items = [] # Example

    result = await passthrough_handler._create_new_subtask(query)
    ...
    passthrough_handler._execute_llm_call.assert_awaited_once_with(
        prompt=query,
...
@pytest.mark.asyncio
async def test_create_new_subtask_template_not_found(passthrough_handler, mocker):
...
@pytest.mark.asyncio
async def test_create_new_subtask_llm_call_fails(passthrough_handler, mocker):
    ...
    passthrough_handler._execute_llm_call = AsyncMock(return_value=failed_llm_result)
...
@pytest.mark.asyncio
async def test_continue_subtask_success(passthrough_handler, mocker):
    ...
    result = await passthrough_handler._continue_subtask(query)
...
@pytest.mark.asyncio
async def test_continue_subtask_llm_call_fails(passthrough_handler, mocker):
...
@pytest.mark.asyncio
async def test_reset_conversation(passthrough_handler):
...
@pytest.mark.asyncio
async def test_command_execution_tool_wrapper_success(passthrough_handler, mocker):
...
@pytest.mark.asyncio
async def test_command_execution_tool_wrapper_failure(passthrough_handler, mocker):
