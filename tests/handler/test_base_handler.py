...
@patch.dict('sys.modules', {
    ...
    'pydantic_ai.models': MagicMock(OpenAIModel=mock_openai_model, AnthropicModel=mock_anthropic_model)
})
...
def mock_command_executor_module():
    ...
    with patch('src.handler.base_handler.command_executor') as mock_module:
        ...
        mock_module.parse_file_paths_from_output = MagicMock(name="parse_file_paths_from_output")
...
def apply_mock_pydantic_ai():
     ...
     with patch('src.handler.base_handler.Agent', mock_agent_class), \
...
def base_handler(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager):
...
def test_init_initializes_components(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager):
    ...
    mock_fm_class.assert_called_once_with(base_path=base_handler_config["file_manager_base_path"])
    ...
    mock_openai_model.assert_called_once_with(api_key=base_handler_config["openai_api_key"], model="gpt-mock")
    mock_agent_class.assert_called_once_with(model=mock_openai_model.return_value, system_prompt=base_handler_config["base_system_prompt"])

@patch('src.handler.base_handler.FileAccessManager')
def test_init_anthropic_model(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager):
    ...
    mock_anthropic_model.assert_called_once_with(api_key=base_handler_config["anthropic_api_key"], model="claude-mock")
    mock_agent_class.assert_called_once_with(model=mock_anthropic_model.return_value, system_prompt=base_handler_config["base_system_prompt"])

@patch('src.handler.base_handler.FileAccessManager')
def test_init_no_model_identifier(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager, caplog):
...
def test_init_missing_api_key(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager, caplog):
...
@patch('src.handler.base_handler.FileAccessManager')
def test_init_unsupported_provider(mock_fm_class, mock_task_system, mock_memory_system, base_handler_config, mock_file_manager, caplog):
...
def test_register_tool_success(base_handler, caplog):
    ...
    def executor_func(inputs): pass

    with caplog.at_level(logging.WARNING):
        result = base_handler.register_tool(tool_spec, executor_func)
        # Check warning about dynamic registration is logged
        assert "dynamic registration with the live pydantic-ai Agent instance is complex" in caplog.text
...
def test_execute_file_path_command_success(base_handler, mock_command_executor_module, base_handler_config):
    ...
    mock_command_executor_module.execute_command_safely.return_value = {
        "success": True, "output": mock_output, "error": "", "exit_code": 0
    }
    mock_command_executor_module.parse_file_paths_from_output.return_value = mock_parsed_paths
    ...
    mock_command_executor_module.execute_command_safely.assert_called_once_with(
        ...
        timeout=base_handler_config["command_executor_timeout"] # Specific timeout from config
    )
    mock_command_executor_module.parse_file_paths_from_output.assert_called_once_with(mock_output)

def test_execute_file_path_command_failure(base_handler, mock_command_executor_module, base_handler_config, caplog):
    """Test command execution failure returns empty list and logs error."""
    command = "badcommand"
    # Configure mocks
    mock_command_executor_module.execute_command_safely.return_value = {
        "success": False, "output": "", "error": "Command not found", "exit_code": 127
    }
...
def test_execute_file_path_command_debug_logging(base_handler, mock_command_executor_module, base_handler_config, caplog):
    ...
    mock_parsed = ["/test/cmd/cwd/debug_test.txt"] # Assuming relative path resolved in cwd
...
def test_reset_conversation(base_handler, caplog):
    ...
    assert "pydantic-ai agent state reset (if applicable) needs verification." in caplog.text
...
def test_set_debug_mode_enables(base_handler, caplog):
    ...
    # Check logs
    assert any(record.levelname == "INFO" and "Debug mode enabled." in record.message for record in caplog.records)
    assert any(record.levelname == "DEBUG" and "[DEBUG] Debug logging is now active." in record.message for record in caplog.records)
...
def test_set_debug_mode_disables(base_handler, caplog):
    ...
    # Check logs
    assert any(record.levelname == "INFO" and "Debug mode disabled." in record.message for record in caplog.records)
    # Ensure the debug message is NOT logged after disabling
    assert not any(record.levelname == "DEBUG" and "[DEBUG] Debug logging is now active." in record.message for record in caplog.records)
     # Add assertions for agent instrumentation mock calls if implemented
...
def test_build_system_prompt_deferred(base_handler):
    ...
    with pytest.raises(NotImplementedError, match="_build_system_prompt implementation deferred"):
        base_handler._build_system_prompt()

def test_get_relevant_files_deferred(base_handler):
    """Verify deferred method raises NotImplementedError."""
    with pytest.raises(NotImplementedError, match="_get_relevant_files implementation deferred"):
        base_handler._get_relevant_files("query")

def test_create_file_context_deferred(base_handler):
    """Verify deferred method raises NotImplementedError."""
    with pytest.raises(NotImplementedError, match="_create_file_context implementation deferred"):
        base_handler._create_file_context([])

def test_execute_tool_deferred(base_handler):
    """Verify deferred method raises NotImplementedError."""
    with pytest.raises(NotImplementedError, match="_execute_tool implementation deferred"):
