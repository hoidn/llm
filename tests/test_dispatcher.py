...
def test_dispatch_s_expression_eval_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system, mock_sexp_evaluator_instance):
    ...
    # Assert the full message generated by _create_failed_result_dict
    expected_message = f"S-expression Evaluation Error: {eval_exception.args[0]}"
    assert result['notes']['error']['message'] == expected_message
...
def test_dispatch_s_expression_syntax_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system, mock_sexp_evaluator_instance):
    ...
    # Assert the full message generated by _create_failed_result_dict
    expected_message = f"S-expression Syntax Error: {str(syntax_exception).split(chr(10))[0]}"
    assert result['notes']['error']['message'] == expected_message
    assert result['notes']['error']['details']['expression'] == identifier
...
def test_dispatch_task_system_execution_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    ...
    fail_error_obj = TaskFailureError(type="TASK_FAILURE", reason=fail_reason, message="Subtask failed", details={"code": 123})
    # Put the *dictionary dump* of the error into the notes of the mock result
    failed_task_result = TaskResult(status="FAILED", content="Subtask failed", notes={"error": fail_error_obj.model_dump(exclude_none=True), "original_note": "abc"})
...
def test_dispatch_handler_tool_execution_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    ...
    fail_error_obj = TaskFailureError(type="TASK_FAILURE", reason=fail_reason, message="Tool crashed", details={"trace": "..."})
    # Put the *dictionary dump* of the error into the notes of the mock result
    failed_tool_result = TaskResult(status="FAILED", content="Tool crashed", notes={"error": fail_error_obj.model_dump(exclude_none=True), "tool_debug": "xyz"})
