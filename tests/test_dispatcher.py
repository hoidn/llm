...
def mock_handler():
    ...
    mock._execute_tool = AsyncMock(return_value=TaskResult(status="COMPLETE", content="Tool Executed", notes={}))
    return mock
...
def mock_task_system():
    ...
    # Convert to AsyncMock
    mock.execute_atomic_template = AsyncMock(return_value=TaskResult(status="COMPLETE", content="Task Executed", notes={}))
    return mock
...
async def test_dispatch_s_expression_routing(
    ...
    result = await execute_programmatic_task(
        ...
        task_system_instance=mock_task_system, 
...
async def test_dispatch_atomic_task_routing(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_direct_tool_routing(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    ...
    result = await execute_programmatic_task(identifier, params, flags, mock_handler, mock_task_system, mock_memory_system)
    mock_task_system.find_template.assert_called_once_with(identifier)
    mock_handler._execute_tool.assert_awaited_once_with(identifier, params)
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_identifier_not_found(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_s_expression_eval_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system, mock_sexp_evaluator_instance):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_s_expression_syntax_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system, mock_sexp_evaluator_instance):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_task_system_execution_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_handler_tool_execution_error(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_direct_tool_continuation_is_failure(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_file_context_parsing_list(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    ...
    mock_task_system.find_template.return_value = template_def
    await execute_programmatic_task(identifier, params, flags, mock_handler, mock_task_system, mock_memory_system)
    mock_task_system.execute_atomic_template.assert_awaited_once()
    call_args = mock_task_system.execute_atomic_template.call_args[0][0]
    assert isinstance(call_args, SubtaskRequest)
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_file_context_parsing_json_string(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    identifier = "atomic:standard:task_with_context"
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_file_context_parsing_invalid_json(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    identifier = "atomic:standard:task_with_context"
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_file_context_parsing_not_list_of_strings(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    identifier = "atomic:standard:task_with_context"
...
@pytest.mark.asyncio
@patch('src.dispatcher.SexpEvaluator')
async def test_dispatch_result_notes_population(MockSexpEvaluatorClass, mock_handler, mock_task_system, mock_memory_system):
    ...
    result_tool = await execute_programmatic_task(identifier_tool, params_tool, flags_tool, mock_handler, mock_task_system, mock_memory_system)
    ...
    result_task = await execute_programmatic_task(identifier_task, params_task, flags_task, mock_handler, mock_task_system, mock_memory_system)
