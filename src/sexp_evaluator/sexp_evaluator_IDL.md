...
module src.sexp_evaluator.sexp_evaluator {
    ...
    interface SexpEvaluator {
        ...
        //     - Parses its *unevaluated* arguments to extract the task name (Symbol), parameter definitions (`(params (p1 type?) ...)`), instructions string (`(instructions "...")`), and optional key-value pairs (e.g., `(subtype "subtask")`, `(description "...")`, `(model "...")`, `(output_format ((type "json") (schema "src.system.models.MyModel")))`, and `(history_config (quote ((use_session_history false) (record_in_session_history true) (history_turns_to_include 5))))` to specify default history management behavior for the defined atomic task).
